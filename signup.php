            ...
            if (isset($_POST['submit'])) {
                ...
                if (!empty($username) && !empty($password1) && !empty($password2) && ($password1 == $password2)) {
                    ...
                    if (mysqli_num_rows($data) == 0) {
                        // The username is unique, so insert the data into the database without hashing the password
                        $query = "INSERT INTO member (username, password, join_date) VALUES ('$username', '$password1', NOW())";
